...
/**
*  A collection of common webdriver methods. For windows, frames, pages manipulation.
 *  Also proxy class for WebDriver.
 */

public abstract class ToolsAbstractModel {

    private static final Logger LOGGER = LoggerFactory.getLogger(ToolsAbstractModel.class);
    ...
    public void logItinerary() {
        logSession("Itinerary#: " + c3ItineraryInfo.getItineraryNumber());
    }

    public void logHotel() {
        logSession(String.format("Hotel name: %s, ID: %s",
                c3HotelSupplyInfo.getFullHotelName(), c3HotelSupplyInfo.getHotelID()));
    }

    public void logCustomer() {
        logSession(String.format("Customer email: %s",
