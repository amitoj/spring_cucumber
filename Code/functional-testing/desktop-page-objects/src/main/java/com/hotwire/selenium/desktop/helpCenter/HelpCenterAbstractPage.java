 ...
 * User: v-ozelenov
 */
public abstract class HelpCenterAbstractPage extends AbstractPageObject {
    ...
    protected HelpCenterAbstractPage(WebDriver webdriver) {
        super(webdriver, By.cssSelector("body"));
    }
     ...
     *
     */
    public HelpCenterAbstractPage(WebDriver webdriver, By keyPageElement) {
        super(webdriver);
        new WebDriverWait(webdriver, PAGE_WAIT)
                .until(ExpectedConditions.visibilityOfElementLocated(keyPageElement));
    }

    public HelpCenterAbstractPage(WebDriver webdriver, By keyPageElement, Integer pageWait) {
        super(webdriver);
    ...
    protected boolean verifyTextOnPageBoolean(String text) {
        try {
            if (findOne(By.xpath("//*[text()[contains(.,'" + text + "')]]"), DEFAULT_WAIT).isDisplayed()) {
    ...
    protected void waitTextLoaded(By byElement, int timeout) {
        logger.info("Waiting for text");
        int count = 0;
        while (findOne(byElement).getText().equals("") || count > timeout) {
