...
@DirtiesContext
public class XnetAriServiceSteps {
    ...
    public void i_m_a_valid_supplier() {
        xnetAriService.setValidSupplier();
    }
    ...
    // @Given("^I want to update every (\\b(?:(?:Sun|Mon|Tues|Wednes|Thurs|Fri|Satur)?day(?:,? *))+\\b) between
    // (\\d+) days from now and (\\d+) days from now$")
